  ...
  ngAfterViewInit() {
    this.testSketch = (s) => {
      ...
      s.setup = () => {
        ...
        ma = s.asin(1/3**0.5)
        s.ortho(-s.width * zoomFactor,s.width * zoomFactor,-s.width* zoomFactor,s.width*zoomFactor,0,10000)
      };
      ...
      s.draw = () => {
        ...
        for(let y = -s.height/2; y< s.height/2; y += this.boxSize){
          for(let x = -s.width/2; x< s.width/2; x += this.boxSize){
            ...
            let h = s.map( (this.noise2D(xT,yT)  + this.noise2D(xT*2,yT*2) / 2 + this.noise2D(xT * 4,yT * 4) / 4) , -1.75,1.75,0,1) ** 2
